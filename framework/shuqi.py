...
from Config import EADHOST, EADPASSWD, OSSPASSWD, OSSUSER, OSSINTERNALENDPOINT, OSSENDPOINT
from config import loadShuQC, loadShuQSeqC, getBloom, loadBloomFromFile, dumpBloomToFile
from easouCrawl import insertCapWithCapObj, getExistsCapsRawUrlId, \
    updateContentById
from executor import getAndParse
from framework.config import OSSPASSWD, OSSUSER, OSSINTERNALENDPOINT, OSSENDPOINT
...
def getBookObjFromSQid(id, shuqCategory):
    ...
    NumChapter = 1
    if root.getiterator('NumChapter') and len(root.getiterator('NumChapter')) > 0 and root.getiterator('NumChapter')[
        0].text:
...
def insertBookWithConn(bookObj, connDoc,csorDoc):
    ...
    sql = 'insert ignore cn_dushu_book_temp ' \
...
def getBucket():
    ...
    endpoint = OSSENDPOINT  # 假设你的Bucket处于杭州区域
